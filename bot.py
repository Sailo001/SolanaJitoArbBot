...
import requests
import logging
from jito_solana.bundle import Bundle
from jito_solana.client import JitoClient
...
JITO_URL = os.getenv('JITO_URL', 'https://mainnet.block-engine.jito.wtf')
JITO_API_KEY = os.getenv('JITO_API_KEY', 'your_jito_api_key')  # Replace with your key
...
WALLET_PRIVATE_KEY = os.getenv('WALLET_PRIVATE_KEY', 'your_private_key')  # Replace with your Solana wallet key
...
class ArbDetector:
    ...
    async def build_jupiter_swap_tx(self, session, quote, input_mint, output_mint, amount, wallet_keypair):
        try:
            params = {
                'quoteResponse': quote,
            ...
            async with session.post("https://quote-api.jup.ag/v6/swap", json=params, timeout=10) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    tx_data = data.get('swapTransaction')
                    if tx_data:
                        tx = Transaction.deserialize(base64.b64decode(tx_data))
                        logger.info(f"Built swap TX for {input_mint} -> {output_mint}")
                        return tx
                    logger.error(f"No swap transaction in Jupiter response")
                    return None
                logger.error(f"Jupiter swap TX failed: {resp.status} - {await resp.text()}")
                return None
        except Exception as e:
            logger.error(f"Error building Jupiter swap TX: {type(e).__name__}: {e}")
            return None

    async def execute_jito_bundle(self, session, quotes, symbol, wallet_keypair):
        try:
            jito_client = JitoClient(JITO_URL, api_key=JITO_API_KEY)
            solana_client = AsyncClient(RPC_URL)

            # Build transactions
            tx1 = await self.build_jupiter_swap_tx(session, quotes['usdc_to_token'], USDC_ADDRESS, self.tokens[symbol]['address'], 100000000, wallet_keypair)
            tx2 = await self.build_jupiter_swap_tx(session, quotes['token_to_usdc'], self.tokens[symbol]['address'], USDC_ADDRESS, quotes['usdc_to_token']['outAmount'], wallet_keypair)
            if not tx1 or not tx2:
                logger.error(f"Failed to build transactions for {symbol}")
                return False

            # Sign transactions
            tx1.sign([wallet_keypair])
            tx2.sign([wallet_keypair])

            # Create Jito bundle
            bundle = Bundle([tx1, tx2])
            bundle_id = await jito_client.submit_bundle(bundle)
            logger.info(f"Submitted Jito bundle for {symbol}: {bundle_id}")

            # Wait for confirmation
            for _ in range(10):
                status = await jito_client.get_bundle_status(bundle_id)
                if status and status.get('status') == 'confirmed':
                    logger.info(f"Jito bundle confirmed for {symbol}: {bundle_id}")
                    return True
                await asyncio.sleep(2)
            logger.warning(f"Jito bundle {bundle_id} for {symbol} not confirmed")
            return False
        except Exception as e:
            logger.error(f"Error executing Jito bundle for {symbol}: {type(e).__name__}: {e}")
            return False
    ...
    async def detect_arb(self, session, symbol, address, amount=100000000):
        ...
        try:
            ...
            if profit_pct >= MIN_PROFIT_PCT:
                ...
                if await self.execute_jito_bundle(session, opp['jup_quote'], symbol, wallet_keypair):
                    logger.info(f"Successfully executed arb for {symbol}")
                    self.notify_telegram(opp)
                else:
                    logger.warning(f"Failed to execute arb for {symbol}")
                return opp
            return None
        except Exception as e:
    ...
    def notify_telegram(self, opp):
        try:
            ...
            msg += f"Executed via Jito bundle: {JITO_URL}"
            if len(msg) > 4096:
