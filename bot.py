...
Uses Moralis API for Raydium data and WebSocket for real-time updates.
"""
...
SOLANA_WS_RPC = os.getenv("SOLANA_WS_RPC", "wss://api.mainnet-beta.solana.com").rstrip()
MORALIS_API_KEY = os.getenv("MORALIS_API_KEY", "")  # Required for Raydium data
...
class ArbDetector:
    ...
    def load_tokens(self):
        ...
        if os.path.isfile(TOKEN_FILE):
            ...
            logger.info("No %s found – will pull pool list from Moralis", TOKEN_FILE)
            self.tokens = {}

    async def fetch_moralis_pools(self, session: aiohttp.ClientSession, needed: int = 300) -> dict:
        """Fetch liquid USDC-paired pools from Moralis API."""
        url = f"{MORALIS_BASE_URL}/token/mainnet/pairs"
        params = {
                ...
                if resp.status == 200:
                    ...
                    for pool in data.get("result", []):
                        ...
                        try:
                            ...
                            continue
                    logger.info("Fetched %s pools from Moralis API", len(out))
                    return out
                else:
                    logger.error("Moralis API error: %s", resp.status)
                    return {}
        except Exception as e:
            logger.error("Failed to fetch Moralis pools: %s", e)
            return {}

    async def subscribe_to_pools(self):
        """Subscribe to pool updates via WebSocket."""
        try:
            self.ws_client = await connect(SOLANA_WS_RPC)
            # Subscribe to Raydium and Orca program accounts
            await self.ws_client.account_subscribe(RAYDIUM_PROGRAM, commitment="confirmed", encoding="base64")
            await self.ws_client.account_subscribe(ORCA_PROGRAM, commitment="confirmed", encoding="base64")
            logger.info("Subscribed to pool updates via WebSocket")
        except Exception as e:
            logger.error("WebSocket subscription failed: %s", e)
    ...
    async def scan(self):
        ...
        async with aiohttp.ClientSession() as session:
            if not self.tokens:
                self.tokens = await self.fetch_moralis_pools(session, needed=300)
                if not self.tokens:
...
async def run_bot():
    ...
    await detector.subscribe_to_pools()  # Initialize WebSocket
    while True:
        try:
            await detector.scan()
        except Exception as e:
