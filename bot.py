...
SPIKE_THRESHOLD = 5.0  # 1h price spike threshold (%)
...
# Map token addresses to CoinGecko IDs (add more as tokens.json grows)
COINGECKO_IDS = {
    ...
    # Note: MOODENG and CHILLGUY addresses need verification
}
...
class ArbDetector:
    def __init__(self):
        ...
        self.spiked_tokens = []  # Tokens with 1h spike
        self.load_tokens()
    ...
    async def is_tradable_on_jupiter(self, session, address):
        try:
            params = {
            ...
            async with session.get(JUPITER_URL, params=params, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    try:
                        ...
                        return False
                logger.warning(f"Jupiter error for {address}: {resp.status} - {await resp.text()}")
                return False
        except Exception as e:
            logger.error(f"Error checking tradability for {address}: {e}")
            return False

    async def detect_spikes(self, session):
        self.spiked_tokens = []
        try:
            params = {
                'vs_currency': 'usd',
                'category': 'solana-ecosystem',
                'order': 'price_change_percentage_1h_in_currency_desc',
                'per_page': 100,
                'page': 1,
                'price_change_percentage': '1h'
            }
            async with session.get(COINGECKO_MARKETS_URL, params=params, timeout=15) as resp:
                if resp.status == 200:
                    coins = await resp.json()
                    for coin in coins:
                        spike = coin.get('price_change_percentage_1h_in_currency', 0)
                        market_cap = coin.get('market_cap', 0)
                        volume_24h = coin.get('total_volume', 0)
                        if (spike > SPIKE_THRESHOLD and 
                            1_000_000 <= market_cap <= 500_000_000 and 
                            volume_24h > 500_000):
                            address = coin.get('contract_address')
                            if address and address in [info['address'] for info in self.tokens.values()]:
                                if await self.is_tradable_on_jupiter(session, address):
                                    symbol = coin['symbol'].upper()
                                    self.spiked_tokens.append(symbol)
                                    logger.info(f"Detected 1h spike for {symbol}: {spike:.2f}% (Market Cap: ${market_cap/1_000_000:.2f}M, Vol: ${volume_24h/1_000_000:.2f}M)")
                else:
                    logger.warning(f"CoinGecko markets fetch failed: {resp.status}")
        except Exception as e:
            logger.error(f"Error detecting spikes: {type(e).__name__}: {e}")
        logger.info(f"Found {len(self.spiked_tokens)} spiked tokens: {self.spiked_tokens}")
    ...
    async def detect_arb(self, session, symbol, address, amount=100000000):
                ...
                if quote2:
                    ...
                    if profit_pct > MIN_PROFIT_PCT:
                        ...
                        if price_diff_pct > 100:
                            price_diff_pct = 0
                        if profit_pct >= 6.0:  # Only notify for +6%
        ...
        # Cross-pair arbitrage: Token A -> Token B -> USDC (for spiked tokens)
        if symbol in self.spiked_tokens:
    ...
    async def scan(self):
        ...
        async with aiohttp.ClientSession() as session:
            await self.detect_spikes(session)
            logger.info(f"Starting arbitrage scan for {len(self.tokens)} tokens (prioritizing {len(self.spiked_tokens)} spiked)")
            tasks = []
            # Prioritize spiked tokens
            for sym in self.spiked_tokens:
            ...
            for sym, info in self.tokens.items():
                if sym not in self.spiked_tokens:
