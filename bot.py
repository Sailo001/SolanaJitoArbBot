#!/usr/bin/env python3
"""
Solana arb bot – executable-opportunity edition
- 3-second poll, 4 concurrent calls (Render-free-tier safe)
- Dynamic size ladder 10 → 100 USDC
- Liquidity ≥ 500 USD, profit ≤ 25 USD, slippage ≤ 1 %
...
MIN_PROFIT_PCT   = float(os.getenv("MIN_PROFIT_PCT", "3"))          # loosened
...
# ---------------------------  env filters  ---------------------------------- #
LIQUIDITY_FLOOR_USD = int(os.getenv("LIQUIDITY_FLOOR_USD", 500))   # loosened
MAX_USD_PROFIT      = int(os.getenv("MAX_USD_PROFIT", 25))         # loosened
...
class ArbDetector:
    ...
    async def detect_one(self, session: aiohttp.ClientSession, symbol: str, address: str) -> Optional[Dict[str, Any]]:
        ...
        for usd_size in (10, 25, 50, 100):                       # dynamic ladder
            ...
            if liquidity_usd < LIQUIDITY_FLOOR_USD:               # env-driven
                continue
            ...
            if max_usd_profit > MAX_USD_PROFIT:                   # env-driven
                continue
