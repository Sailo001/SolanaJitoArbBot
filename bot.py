...
CYCLE_TYPE = os.getenv('CYCLE_TYPE', 'direct').lower()  # 'triangle' or 'direct'
...
# Rate limiter: 0.5 requests per second (120 calls/min, safer for Jupiter)
RATE_LIMIT = asyncio.Semaphore(0.5)
...
class ArbDetector:
    ...
    async def get_quote(self, session, input_mint, output_mint, amount, retries=5, use_orca=False):
        ...
        if use_orca:
            url = ORCA_URL
            params = {}  # Simplified; refine for Orca
        else:
            url = JUPITER_URL
            params = {
                ...
                try:
                    async with session.get(url, params=params, timeout=15) as resp:
                        if resp.status == 200:
                            ...
                            if use_orca:
                                logger.debug(f"Got Orca quote for {input_mint} -> {output_mint}")
                                return {'outAmount': data.get('amountOut', 0)}  # Mock structure
    ...
    async def detect_arb(self, session, symbol, address, amount=50000000):  # 50 USDC
        ...
        try:
            if CYCLE_TYPE == 'direct':
                ...
                if not quote1:
                    quote1 = await self.get_quote(session, USDC_ADDRESS, address, amount, use_orca=True)
                    if not quote1:
                ...
                if not quote2:
                    quote2 = await self.get_quote(session, address, USDC_ADDRESS, token_out, use_orca=True)
                    if not quote2:
                ...
                if not quote2:
                    quote2 = await self.get_quote(session, address, SOL_ADDRESS, token_out, use_orca=True)
                    if not quote2:
                ...
                if not quote3:
                    quote3 = await self.get_quote(session, SOL_ADDRESS, USDC_ADDRESS, sol_out, use_orca=True)
                    if not quote3:
            ...
            raise  # Re-raise for scan task error logging
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for sym, info in self.tokens.items():
                if info['address'] not in [p[1] for p in self.invalid_tokens]:
                    ...
                    tasks.append(task)
                    await asyncio.sleep(random.uniform(2.0, 3.0))  # Increased delay to avoid bursts
