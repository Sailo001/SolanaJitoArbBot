...
class ArbDetector:
    ...
    def load_tokens(self):
        """Initialize empty token list; populated via refresh_tokens"""
        self.tokens = {}
        logger.info("Initialized empty token list; awaiting refresh_tokens")
    ...
    async def scan(self):
        if not self.tokens:
            logger.warning("No tokens to scan; refreshing tokens")
            async with aiohttp.ClientSession() as session:
                await self.refresh_tokens(session)
    ...
    async def refresh_tokens(self, session, limit=50):
        ...
        try:
            ...
            # Fallback to minimal token set if fetch fails
            self.tokens = {
            ...
            logger.info(f"Fallback to {len(self.tokens)} default tokens")
