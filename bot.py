...
Zero-config version – pulls trending Solana mints from DexScreener
...
class ArbDetector:
    ...
    def load_tokens(self):
        ...
        Otherwise leave self.tokens empty; scan() will pull from DexScreener.
        """
        if os.path.isfile(TOKEN_FILE):
            ...
            logger.info("No %s found – will pull from DexScreener at first scan", TOKEN_FILE)
            self.tokens = {}

    # ------------------------------------------------------------------
    # Fetch trending Solana mints from DexScreener (validated)
    # ------------------------------------------------------------------
    async def fetch_trending_solana_mints(self, session: aiohttp.ClientSession, limit: int = 15) -> dict:
        url = DEXSCREENER_URL
        try:
            async with session.get(url, timeout=15) as resp:
                if resp.status != 200:
                    logger.warning("DexScreener HTTP %s", resp.status)
                    return {}
    ...
    # keep the rest
                ...
                try:  # validate immediately
                    Pubkey.from_string(mint)
                    out[symbol] = {"address": mint}
                except Exception:
                    logger.debug("Skipping bad mint %s", mint)
            logger.info("Fetched %s trending Solana mints from DexScreener", len(out))
            return out
    ...
    async def scan(self):
        async with aiohttp.ClientSession() as session:
            if not self.tokens:  # first run or no file
                self.tokens = await self.fetch_trending_solana_mints(session, limit=12)
                if not self.tokens:
