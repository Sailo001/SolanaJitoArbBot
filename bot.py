...
POLL_INTERVAL = int(os.getenv('POLL_INTERVAL', 60))  # Increased to 60s to reduce rate limits
...
class ArbDetector:
    ...
    async def get_quote(self, session, input_mint, output_mint, amount, retries=5):  # Increased to 5
    ...
    async def scan(self):
        ...
        async with aiohttp.ClientSession() as session:
            # NEW: Sequential processing with random delay to avoid burst 429s
            tasks = []
            for sym, info in self.tokens.items():
                if info['address'] not in [p[1] for p in self.invalid_tokens]:
                    task = self.detect_arb(session, sym, info['address'])
                    tasks.append(task)
                    await asyncio.sleep(random.uniform(0.5, 1.5))  # Random delay between tasks
