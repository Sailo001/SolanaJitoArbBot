...
ORCA_URL = "https://api.orca.so/pools"  # Fallback API

# Constants (moved to module level for global access)
...
class ArbDetector:
    ...
    async def get_quote(self, session, input_mint, output_mint, amount, retries=5, use_orca=False):
        ...
        if use_orca:
            url = ORCA_URL
            params = {}  # Orca requires pool queries; simplified for fallback
        else:
            url = JUPITER_URL
            params = {
                ...
                try:
                    async with session.get(url, params=params, timeout=15) as resp:
                        if resp.status == 200:
                            ...
                            if use_orca:
                                # Simplified Orca response parsing (adjust based on API)
                                logger.debug(f"Got Orca quote for {input_mint} -> {output_mint}")
                                return {'outAmount': data.get('amountOut', 0)}  # Mock structure
                            logger.debug(f"Got Jupiter quote for {input_mint} -> {output_mint}")
                            return data
    ...
    async def detect_arb(self, session, symbol, address, amount=10000000):  # 10 USDC for larger trades
        ...
        if address in [pair[1] for pair in self.invalid_tokens]:
            ...
            return None
        try:
            # USDC -> Token (try Jupiter, fallback to Orca)
            ...
            logger.info(f"Profit for {symbol}: {profit_pct:.2f}%")  # Changed to INFO for visibility
