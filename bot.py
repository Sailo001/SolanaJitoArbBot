...
from raydium import Liquidity  # For Raydium pool data
...
class ArbDetector:
    ...
    async def get_raydium_pool_price(self, token_address, retries=3):
        ...
        try:
            ...
            pool_id = Liquidity.get_pool_id(token_mint, usdc_mint, RAYDIUM_PROGRAM)
            for attempt in range(retries):
                try:
                    pool_info = await Liquidity.fetch_pool(self.rpc_client, pool_id)
                    if not pool_info:
                        logger.warning(f"No Raydium pool data for {token_address}")
                        return None
                    reserve_token = pool_info['baseReserve'] if token_mint < usdc_mint else pool_info['quoteReserve']
                    reserve_usdc = pool_info['quoteReserve'] if token_mint < usdc_mint else pool_info['baseReserve']
                    price = reserve_usdc / reserve_token if reserve_token else 0
                    logger.debug(f"Raydium price for {token_address}: {price}")
                    return price
                except Exception as e:
    ...
    async def get_orca_pool_price(self, token_address, retries=3):
        """Fetch price from Orca Whirlpool (simplified)"""
        try:
            # Resolve Orca pool address (simplified, assumes known pool)
            token_mint = Pubkey.from_string(token_address)
            usdc_mint = Pubkey.from_string(USDC_ADDRESS)
            # Orca pool address derivation requires Whirlpool program specifics
            # Placeholder: Assume pool address is fetched via Orca SDK or known
            pool_address = Pubkey.from_string(token_address)  # TODO: Replace with actual Orca pool resolution
            for attempt in range(retries):
                try:
                    response = await self.rpc_client.get_account_info_json_parsed(pool_address)
                    if not response.value:
                        logger.warning(f"No Orca pool data for {token_address}")
                        return None
                    # Simplified: Parse pool reserves (requires Orca Whirlpool layout)
                    data = response.value.data.parsed['info']
                    reserve_token = data['tokenAmountA'] if token_mint < usdc_mint else data['tokenAmountB']
                    reserve_usdc = data['tokenAmountB'] if token_mint < usdc_mint else data['tokenAmountA']
                    price = reserve_usdc / reserve_token if reserve_token else 0
                    logger.debug(f"Orca price for {token_address}: {price}")
                    return price
                except Exception as e:
