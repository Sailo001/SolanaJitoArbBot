#!/usr/bin/env python3
"""
Solana arb bot – Render-console-visible edition
- Background scanner prints every cycle
- /start-logs endpoint keeps container awake (free tier)
- All previous filters (liquidity, slippage, sim-tx) still active
"""
...
class ArbDetector:
    ...
    def _build_simple_jup_tx(self, q1: Dict[str, Any], q2: Dict[str, Any]) -> str:
        return base64.b64encode(json.dumps({"route": [q1, q2]}).encode()).decode()
    ...
    def notify_telegram(self, opp: Dict[str, Any]):
        try:
            msg = f"Arb: {opp['symbol']} | Profit: {opp['profit_pct']:.2f}% | Time: {datetime.now()}\nCoinGecko: {opp['cg_price'] or 'N/A'}\nExecute: https://jup.ag/swap"
...
def test_telegram():
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
        resp = requests.get(url, params={"chat_id": TELEGRAM_CHAT_ID, "text": "Test from unlimited-scan bot"}, timeout=20)
...
def start_logs():
    """Ping this once to keep container awake; logs will appear in Render console."""
    logger.info("KEEP-ALIVE: /start-logs hit – container stays awake")
    return jsonify(status="logs activated – scan lines will print every {} s".format(POLL_INTERVAL))

# --------------------------------------------------------------------------- #
# runner
# --------------------------------------------------------------------------- #
KEEP_ALIVE = True   # set False if you switch to background-worker plan
...
if __name__ == "__main__":
    logger.info("Unlimited-token Solana arb bot starting – logs visible via /start-logs")
