...
CYCLE_TYPE = os.getenv('CYCLE_TYPE', 'triangle').lower()  # 'triangle' or 'direct'
...
class ArbDetector:
    ...
    async def get_quote(self, session, input_mint, output_mint, amount, retries=5, use_orca=False):
        ...
        if use_orca:
            url = ORCA_URL
            params = {}  # Simplified; refine for Orca pool queries
        else:
            url = JUPITER_URL
            params = {
        ...
        for attempt in range(retries):
            async with RATE_LIMIT:
                try:
                    ...
                    logger.error(f"{'Orca' if use_orca else 'Jupiter'} quote error for {input_mint} -> {output_mint}: {e}")
