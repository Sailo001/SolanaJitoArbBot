...
-------------------------------------------------------------------------------
- Pulls live Jupiter **strict** token-list every 5 min  ->  ~1 600 SPL addresses
- Still respects MAX_CONCURRENT_REQUESTS=2  ->  no 429 storms
- Still uses only free endpoints:
      Jupiter /strict  (cached 5 min)
      Jupiter /v6/quote
...
JUP_TOKENS_URL   = "https://token.jup.ag/strict"          # working endpoint
...
async def coingecko_price(session: aiohttp.ClientSession, address: str) -> Optional[float]:
    # minimal mapper – expand as needed
    mapper = {
