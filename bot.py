...
Zero-config – pulls 300+ liquid USDC-paired pools from Raydium **static GitHub JSON**
...
class ArbDetector:
    ...
    def load_tokens(self):
        """Use tokens.json if it exists; otherwise leave empty -> scan() will populate."""
        if os.path.isfile(TOKEN_FILE):
            with open(TOKEN_FILE) as f:
                self.tokens = json.load(f)
                logger.info("Loaded %s tokens from %s", len(self.tokens), TOKEN_FILE)
        else:
            logger.info("No %s found – will pull large pool list at first scan", TOKEN_FILE)
            self.tokens = {}

    # ------------------------------------------------------------------
    # Large liquid pool list  (Raydium static GitHub JSON)  +  DexScreener fallback
    # ------------------------------------------------------------------
    async def fetch_trending_solana_mints(self, session: aiohttp.ClientSession, needed: int = 300) -> dict:
        # 1️⃣  Raydium static GitHub JSON (always online, 2 000+ USDC-paired pools)
            ...
            async with session.get(BIG_LIST, timeout=15) as resp:
                if resp.status == 200:
                    ...
                    for pool in data:
                        ...
                        if liquidity < 2_000:          # ≥ $2 k  (lower if you wish)
                            continue
                        mint = pool["baseMint"]
