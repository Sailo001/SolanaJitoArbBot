...
class ArbDetector:
    ...
    async def get_pool_price(self, session, token_address, dex_program, retries=3):
        """Fetch token price from a DEX pool (Raydium or Orca)"""
        try:
            pool_address = None  # Simplified: Assume pool address is known or derived
            for attempt in range(retries):
                try:
                    # Example: Fetch pool state (reserves) via Solana RPC
                    response = await self.rpc_client.get_account_info_json_parsed(Pubkey.from_string(token_address))
                    if not response.value:
                        logger.warning(f"No pool data for {token_address} on {dex_program}")
                        return None
                    # Simplified price calculation (reserve_token / reserve_usdc)
                    data = response.value.data
                    reserve_token = data.get('reserve_token', 0)  # Placeholder
                    reserve_usdc = data.get('reserve_usdc', 1)  # Avoid division by zero
                    price = reserve_usdc / reserve_token if reserve_token else 0
                    logger.debug(f"Got {dex_program} price for {token_address}: {price}")
                    return price
                except Exception as e:
                    logger.error(f"Error fetching {dex_program} price for {token_address}: {type(e).__name__}: {e}")
            ...
            logger.error(f"Pool price error for {token_address}: {type(e).__name__}: {e}")
            return None
    ...
    async def detect_arb(self, session, symbol, address, amount=100000000):
        ...
        try:
            ...
            raydium_price = await self.get_pool_price(session, address, RAYDIUM_PROGRAM)
            orca_price = await self.get_pool_price(session, address, ORCA_PROGRAM)
