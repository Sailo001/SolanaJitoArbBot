...
- 3-second poll, 4 concurrent calls (Render-free-tier safe)
- Dynamic size ladder 10 → 100 USDC
- Liquidity ≥ 200 USD, profit ≤ 25 USD, slippage ≤ 1 %
- Real /swap tx → simulateTransaction
"""
...
MIN_PROFIT_PCT   = float(os.getenv("MIN_PROFIT_PCT", "1"))          # 1 % floor
...
# ---------------------------  env filters  ---------------------------------- #
LIQUIDITY_FLOOR_USD = int(os.getenv("LIQUIDITY_FLOOR_USD", 200))   # ≥ 200 USD
MAX_USD_PROFIT      = int(os.getenv("MAX_USD_PROFIT", 25))         # ≤ 25 USD
...
def test_telegram():
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
        resp = requests.get(url, params={"chat_id": TELEGRAM_CHAT_ID, "text": "Dynamic-size bot"}, timeout=20)
...
def start_logs():
    ...
    return jsonify(status="logs active every {} s".format(POLL_INTERVAL))
