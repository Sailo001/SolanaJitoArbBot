#!/usr/bin/env python3
"""
Solana arb bot – RAW-QUOTE diagnostic edition
Prints every quote (no filters) for first 10 tokens per sweep
"""
...
MIN_PROFIT_PCT   = float(os.getenv("MIN_PROFIT_PCT", "0.5"))        # ultra-loose for test
...
class ArbDetector:
    ...
    async def detect_one(self, session: aiohttp.ClientSession, symbol: str, address: str) -> Optional[Dict[str, Any]]:
        ...
        raw_profit_pct = ((int(q2["outAmount"]) - amount) / amount) * 100
        logger.info("RAW  %s  %.2f%%  in=%s  out=%s", symbol, raw_profit_pct, amount, q2["outAmount"])
    ...
    # no Telegram in diagnostic mode
...
def test_telegram():
    try:
        ...
        resp = requests.get(url, params={"chat_id": TELEGRAM_CHAT_ID, "text": "Diagnostic raw-quote bot"}, timeout=20)
