...
MAX_CONCURRENT_REQUESTS = 3  # Reduced to avoid 429
                ...
                try:
                    async with session.get(JUPITER_URL, params=params, timeout=15) as resp:  # Increased timeout
                        if resp.status == 200:
                            ...
                            wait_time = 2 ** (attempt + 1)  # Exponential backoff: 2s, 4s, 8s, 16s, 32s
        ...
        for attempt in range(retries):
            async with RATE_LIMIT:
                try:
                    ...
                    async with session.get(COINGECKO_URL, params=params, timeout=15) as resp:  # Increased timeout
                        if resp.status == 200:
                            text = await resp.text()
                            if not text:
    ...
    async def scan(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for sym, info in self.tokens.items():
                tasks.append(asyncio.wait_for(self.detect_arb(session, sym, info['address']), timeout=20))  # Increased timeout
            valid_results = 0
