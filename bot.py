...
Uses DexScreener API for token discovery and on-chain methods for real-time pricing.
"""
...
class ArbDetector:
    ...
    async def fetch_dexscreener_pools(self, session: aiohttp.ClientSession, needed: int = 300) -> dict:
        """Fetch liquid USDC-paired pools from DexScreener API."""
        url = DEXSCREENER_URL
        params = {"q": "solana", "rankBy": "liquidity", "limit": 100}
            ...
            async with session.get(url, params=params, timeout=15) as resp:
                if resp.status == 200:
                    ...
                    for pair in data.get("pairs", []):
                        if pair.get("quoteToken", {}).get("address") != USDC_ADDRESS:
                            continue
    ...
    async def get_coingecko_price(self, session: aiohttp.ClientSession, token_address: str):
        ...
        try:
            ...
            logger.error("CoinGecko price failed: %s", e)
            return None
    ...
    async def scan(self):
        ...
        async with aiohttp.ClientSession() as session:
            if not self.tokens:
                self.tokens = await self.fetch_dexscreener_pools(session, needed=300)
                if not self.tokens:
