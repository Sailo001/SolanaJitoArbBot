...
from solders.account_decoder import parse_token
    ...
    async def get_raydium_pool_price(self, token_address, retries=3):
        ...
        try:
            ...
            for attempt in range(retries):
                try:
                    response = await self.rpc_client.get_account_info_json_parsed(pool_address, commitment=Confirmed)
                    if not response.value:
                        logger.warning(f"No Raydium pool data for {token_address} at {pool_address}")
                        return None
                    # Parse Raydium AMM pool data (simplified)
                    data = response.value.data
                    # Assume standard Raydium AMM layout: baseReserve and quoteReserve at fixed offsets
                    # Note: Actual offsets depend on Raydium's AMM struct; this is simplified
                    reserve_token = int.from_bytes(data[100:108], 'little')  # Example offset
                    reserve_usdc = int.from_bytes(data[108:116], 'little')   # Example offset
                    if reserve_token == 0 or reserve_usdc == 0:
                        logger.warning(f"Invalid reserves for {token_address} on Raydium")
                        return None
        ...
        try:
            ...
            for attempt in range(retries):
                try:
                    ...
                    reserve_token = int.from_bytes(data[64:72], 'little')  # Example offset
                    reserve_usdc = int.from_bytes(data[72:80], 'little')   # Example offset
                    if reserve_token == 0 or reserve_usdc == 0:
                        logger.warning(f"Invalid reserves for {token_address} on Orca")
                        return None
