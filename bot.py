...
class ArbDetector:
    ...
    async def get_quote(self, session, input_mint, output_mint, amount, retries=5):
        ...
        for attempt in range(retries):
            async with RATE_LIMIT:  # Enforce 1 request/second
                try:
    ...
    async def get_raydium_price(self, session, token_address):
        if not USE_RAYDIUM:
            ...
            return None
        async with RATE_LIMIT:  # Apply rate limit to Raydium
            try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for sym, info in self.tokens.items():
                if info['address'] not in [p[1] for p in self.invalid_tokens]:
                    ...
                    tasks.append(task)
                    await asyncio.sleep(random.uniform(1.0, 2.0))  # Increased delay
