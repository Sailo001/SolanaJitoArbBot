...
class ArbDetector:
        ...
        try:
            with open(TOKEN_FILE, "r") as f:
                ...
                if not isinstance(self.tokens, dict):
                    raise ValueError("tokens.json must be a dict")
                for sym, info in self.tokens.items():
                    if not isinstance(info, dict) or "address" not in info:
                        raise ValueError(f"Invalid entry for {sym}")
            ...
            async with RATE_LIMIT:
                try:
                    async with session.get(JUPITER_URL, params=params, timeout=20) as resp:
                        ...
                        if resp.status == 429:
                            wait = 2**attempt * 4
                            logger.warning(f"Jupiter 429 – wait {wait}s")
                            await asyncio.sleep(wait)
                            continue
                        if resp.status == 400:
                            self.invalid_tokens[pair] = time.time()
                            return None
                        logger.warning(f"Jupiter {resp.status} – {await resp.text()}")
                        return None
                except asyncio.TimeoutError:
                    logger.warning(f"Jupiter timeout (attempt {attempt+1})")
                    if attempt < retries - 1:
                        await asyncio.sleep(2**attempt * 4)
                except Exception as e:
    ...
    async def detect_arb(
        ...
        if address == USDC_ADDRESS:
            return None
        logger.info(f"Checking arb for {symbol}")
        ...
        logger.info(f"{symbol} profit: {profit_pct:.2f}% (buy {buy_dex}, sell {sell_dex})")

        if profit_pct >= MIN_PROFIT_PCT:
            # Validate with Jupiter round-trip
            ...
            logger.info(f"ARB FOUND: {symbol} {profit_pct:.2f}%")
    ...
    async def scan(self):
        self.load_tokens()
        logger.info(f"Starting scan for {len(self.tokens)} tokens")
        async with aiohttp.ClientSession() as session:
            ...
            logger.info(f"Scan complete – {valid} opportunities")
    ...
    # ------------------------------------------------------------------
    # DexScreener refresh (trending Solana tokens)
    # ------------------------------------------------------------------
    async def refresh_tokens(self, session: aiohttp.ClientSession, limit=10):
        try:
            async with session.get(DEXSCREENER_URL, timeout=20) as resp:
                if resp.status != 200:
                    logger.error(f"DexScreener {resp.status}")
                    return
                data = await resp.json()
                new_tokens = {}
                for pair in data.get("pairs", [])[:limit]:
                    if pair["chainId"] == "solana":
                        ...
                        if addr in COINGECKO_IDS:
                            new_tokens[pair["baseToken"]["symbol"]] = {"address": addr}
                if new_tokens:
                    with open(TOKEN_FILE, "w") as f:
                        json.dump(new_tokens, f, indent=2)
                    self.load_tokens()
                    logger.info(f"Refreshed {len(new_tokens)} tokens")
        except Exception as e:
            logger.error(f"DexScreener refresh error: {e}")
