...
USE_RAYDIUM = os.getenv('USE_RAYDIUM', 'false').lower() == 'true'
...
ORCA_QUOTE_URL = "https://api.orca.so/v1/quote"

# Constants
SOL_ADDRESS = 'So11111111111111111111111111111111111111112'
USDC_ADDRESS = 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v'

# Rate limiter: 0.3 requests per second (18 calls/min)
...
class ArbDetector:
    ...
    async def get_quote(self, session, input_mint, output_mint, amount, retries=5, use_orca=False):
        ...
        for attempt in range(retries):
            async with RATE_LIMIT:
                try:
                    async with session.get(url, params=params, timeout=15) as resp:
                        if resp.status == 200:
                            ...
                            if use_orca:
                                ...
                                if amount_out == 0:
                                    logger.warning(f"No valid Orca quote for {input_mint} -> {output_mint}: {data}")
                                    return None
                                logger.debug(f"Got Orca quote for {input_mint} -> {output_mint}: {amount_out} lamports")
                                return {'outAmount': amount_out}
                            logger.debug(f"Got Jupiter quote for {input_mint} -> {output_mint}: {data['outAmount']} lamports")
                            return data
                        elif resp.status == 429:
                            wait_time = 2 ** attempt
                            logger.warning(f"{'Orca' if use_orca else 'Jupiter'} rate limit (429) for {input_mint} -> {output_mint}. Retrying in {wait_time}s (attempt {attempt+1}/{retries})")
                            await asyncio.sleep(wait_time)
                        elif resp.status == 400:
                            error_text = await resp.text()
                            logger.warning(f"{'Orca' if use_orca else 'Jupiter'} bad request (400) for {input_mint} -> {output_mint}: {error_text}")
                            if not use_orca:
                                logger.info(f"Attempting Orca quote for {input_mint} -> {output_mint}")
                                return await self.get_quote(session, input_mint, output_mint, amount, retries=retries, use_orca=True)
                            self.invalid_tokens.add((input_mint, output_mint))
                            logger.info(f"Marked {input_mint} -> {output_mint} as invalid")
                            return None
                        else:
                            logger.warning(f"{'Orca' if use_orca else 'Jupiter'} quote failed: {resp.status} for {input_mint} -> {output_mint}: {await resp.text()}")
                            return None
                except RuntimeError as e:
                    if "Session is closed" in str(e):
                        logger.error(f"Session closed for {input_mint} -> {output_mint}: {e}. Retrying with new session.")
                        # Create new session for retry
                        async with aiohttp.ClientSession() as new_session:
                            return await self.get_quote(new_session, input_mint, output_mint, amount, retries=1, use_orca=use_orca)
                    else:
                        logger.error(f"RuntimeError for {input_mint} -> {output_mint}: {e}")
                        return None
                except asyncio.TimeoutError as e:
                    logger.error(f"{'Orca' if use_orca else 'Jupiter'} timeout for {input_mint} -> {output_mint}: {e}")
                    ...
                    logger.error(f"{'Orca' if use_orca else 'Jupiter'} response error for {input_mint} -> {output_mint}: {e}")
                    return None
                except Exception as e:
                    logger.error(f"{'Orca' if use_orca else 'Jupiter'} unexpected error for {input_mint} -> {output_mint}: {type(e).__name__}: {e}")
                    return None
    ...
    async def detect_arb(self, session, symbol, address, amount=100000000):  # 100 USDC
        ...
        try:
                ...
                if ray_price is None:
                    ...
                    return {
                        ...
                        'jup_quote': {
                            ...
                            'token_to_sol': quote2 if CYCLE_TYPE == 'triangle' else None,
                            'sol_to_usdc': quote3 if CYCLE_TYPE == 'triangle' else None,
                            'token_to_usdc': quote2 if CYCLE_TYPE == 'direct' else None
                        },
                        'ray_price': None
                    }
            ...
            logger.error(f"Timeout detecting arb for {symbol}: {e}")
            return None  # Skip token
        except RuntimeError as e:
            if "Session is closed" in str(e):
                logger.error(f"Session closed for {symbol}: {e}. Skipping token.")
                return None
            else:
                logger.error(f"RuntimeError for {symbol}: {e}")
                return None
        except Exception as e:
    ...
    async def scan(self):
        ...
        async with aiohttp.ClientSession() as session:  # Single session for the entire scan
            tasks = []
    ...
    def notify_telegram(self, opp):
        try:
            msg = f"Arb Opportunity: {opp['symbol']} | Profit: {opp['profit_pct']:.2f}% | Time: {datetime.now()} | Cycle: {CYCLE_TYPE}\n"
