...
Zero-config – pulls 300+ liquid USDC-paired pools from Raydium API v2
...
class ArbDetector:
    ...
    # ------------------------------------------------------------------
    # Large liquid pool list  (Raydium API v2)  +  DexScreener fallback
    # ------------------------------------------------------------------
    async def fetch_trending_solana_mints(self, session: aiohttp.ClientSession, needed: int = 300) -> dict:
        # 1️⃣  Raydium official API v2  (no key, 1000+ USDC-paired pools)
