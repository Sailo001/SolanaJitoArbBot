...
const tokens = [];
const pnlHistory = [];
const scannedTokens = new Set(); // tracks tokens already alerted
...
async function scanTokens() {
  ...
  for (const token of tokens) {
    if (scannedTokens.has(token)) continue; // skip already alerted tokens

    const pnl = (Math.random() * 20 - 10).toFixed(2); // -10 to +10
    pnlHistory.push({ token, pnl });
    scannedTokens.add(token);

    // Telegram alert with inline controls
...
// Automatic scanning every interval
const SCAN_INTERVAL = 15000; // 15 seconds
setInterval(scanTokens, SCAN_INTERVAL);
