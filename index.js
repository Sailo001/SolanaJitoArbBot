...
async function jupQuote(inputMint, outputMint, amountLamports, retries = 3) {
  // TRIMMED to avoid the trailing-space bug
...
// --------------  BUILD ROUTES  (0.3 SOL) --------------
async function buildBestRoutes(tokenMint, amountSol = 0.3) {
  const decimals = await getTokenDecimals(tokenMint);
  if (!decimals) return [];

  const lamports   = Math.floor(amountSol * 10 ** decimals); // token lamports
  const solLamports= Math.floor(amountSol * 1e9);            // 0.3 SOL
...
// --------------  EXECUTE  --------------
async function executeFlashloanTrade(tokenMint, buyDex, sellDex, amountSol = 0.3) {
  try {
    const decimals = await getTokenDecimals(tokenMint);
