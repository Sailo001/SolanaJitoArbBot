...
async function getSwapInstructions(tokenMint, buyDex, sellDex, amount) {
  try {
    ...
    const buyUrl = `https://quote-api.jup.ag/v6/quote?inputMint=${baseMint}&outputMint=${tokenMint}&amount=${amountIn}&slippageBps=${SLIPPAGE_BPS}&onlyDirectRoutes=true&dexes=${buyDex}`;
    const sellUrl = `https://quote-api.jup.ag/v6/quote?inputMint=${tokenMint}&outputMint=${baseMint}&amount=${amountIn}&slippageBps=${SLIPPAGE_BPS}&onlyDirectRoutes=true&dexes=${sellDex}`;

    logger.info(`Fetching swap routes: buy ${buyUrl}, sell ${sellUrl}`);
    const [buyRes, sellRes] = await Promise.all([fetch(buyUrl), fetch(sellUrl)]);
    if (!buyRes.ok || !sellRes.ok) {
      logger.error(`Jupiter API error: Buy ${buyRes.status}, Sell ${sellRes.status}`);
      throw new Error(`HTTP ${buyRes.status || sellRes.status}`);
    }
    ...
    logger.info(`Swap routes fetched for mint ${tokenMint}`);
    return {
      buyRoute: buyData.data[0],
      sellRoute: sellData.data[0],
      buyOutAmount: buyData.data[0].outAmount / 1e9,
      sellOutAmount: sellData.data[0].outAmount / 1e9
    };
  } catch (err) {
    logger.error(`Error fetching swap instructions for ${tokenMint}: ${err.message}`);
    return null;
  }
}
...
async function executeFlashloanTrade(tokenMint, buyDex, sellDex, amount) {
  try {
    ...
    // Calculate fees
    const flashloanFee = (amount * FLASHLOAN_FEE_BPS) / 10000;
    ...
    // Add flashloan repay
    transaction.add(await createFlashRepayInstruction(connection, amount + flashloanFee, wallet.publicKey));
...
bot.on('text', async ctx => {
  ...
  const estimatedProfit = routes
    ? routes.sellOutAmount - routes.buyOutAmount - (TX_FEE_PER_SIGNATURE * 3 + (1 * FLASHLOAN_FEE_BPS) / 10000 + JITO_TIP_FEE)
    : null;
  ...
  ctx.reply(
    ...
    `Sell on: *${sellDex}*\n` +
    `Fees: ${TX_FEE_PER_SIGNATURE * 3} SOL (tx), ${FLASHLOAN_FEE_BPS / 100}% flashloan, ${JITO_TIP_FEE} SOL (Jito)\n` +
    (estimatedProfit ? `Estimated Profit: ${estimatedProfit.toFixed(6)} SOL` : '⚠️ Unable to estimate profit'),
    {
      parse_mode: 'Markdown',
