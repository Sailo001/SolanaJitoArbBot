...
const SPREAD_THRESHOLD = parseFloat(process.env.SPREAD_THRESHOLD || "1"); // % threshold
const SCAN_INTERVAL = parseInt(process.env.SCAN_INTERVAL || "25000"); // 25s
...
// ===== Continuous round-robin token batch =====
let currentIndex = 0;
async function getNextTokenBatch() {
  const batch = [];
  const totalTokens = allTokens.length;
  for (let i = 0; i < BATCH_SIZE; i++) {
    const token = allTokens[currentIndex];
    batch.push(token);
    currentIndex = (currentIndex + 1) % totalTokens;
  }
  return batch;
}
...
async function scanArbitrage() {
  const tokensBatch = await getNextTokenBatch();
...
sendTelegramMessage(
  `✅ Bot deployed with full token scanning + real-time PnL!\n` +
