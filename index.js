...
async function getSwapInstructions(tokenMint, buyDex, sellDex, amount) {
  try {
    ...
    // Use token decimals for amount (RAY has 6 decimals, SOL has 9)
    const amountIn = amount * 10 ** decimals;
    ...
    if (!buyRes.ok || !sellRes.ok) {
      logger.error(`Jupiter API error: Buy ${buyRes.status} (${buyText}), Sell ${sellRes.status} (${sellText})`);
      throw new Error(`HTTP ${buyRes.status || sellRes.status}: ${buyText || sellText}`);
    }

    const [buyData, sellData] = [JSON.parse(buyText), JSON.parse(sellText)];
    if (!buyData.data?.length || !sellData.data?.length) {
      logger.error(`No routes found for mint ${tokenMint}, buyDex ${buyDex}, sellDex ${sellDex}`);
      throw new Error(`No routes found: Buy ${JSON.stringify(buyData)}, Sell ${JSON.stringify(sellData)}`);
    }
...
bot.on('text', async ctx => {
  ...
  ctx.reply(
    ...
    `Sell on: *${sellDex}*\n` +
    `Fees: ${TX_FEE_PER_SIGNATURE * 3} SOL (tx), ${FLASHLOAN_FEE_BPS / 100}% flashloan, ${JITO_TIP_FEE} SOL (Jito)\n` +
    (estimatedProfit ? `Estimated Profit: ${estimatedProfit.toFixed(6)} SOL` : `⚠️ Unable to estimate profit: ${routes.error || 'Unknown error'}`),
    {
      parse_mode: 'Markdown',
