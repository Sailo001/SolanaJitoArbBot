...
const logger = winston.createLogger({
  ...
  transports: [
    ...
    new winston.transports.Console() // Add stdout for Render dashboard
  ]
});
...
async function getSwapInstructions(tokenMint, buyDex, sellDex, amount, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // Use smaller amount (0.1 tokens)
      const amountIn = Math.floor(amount * 10 ** decimals * 0.1);
      ...
      logger.info(`Attempt ${attempt}: Fetching swap routes`, { buyUrl, sellUrl });
      ...
      if (!buyRes.ok || !sellRes.ok) {
        logger.error(`Jupiter API error (attempt ${attempt})`, {
      ...
      try {
        ...
        logger.error(`JSON parse error (attempt ${attempt})`, { buyText, sellText, error: err.message });
        throw new Error(`Invalid JSON response: ${err.message}`);
      }

      if (!buyData.data?.length || !sellData.data?.length) {
        logger.error(`No routes found for mint ${tokenMint}, buyDex ${buyDex}, sellDex ${sellDex}`, {
          buyData,
          sellData
        });
...
async function executeFlashloanTrade(tokenMint, buyDex, sellDex, amount) {
  try {
    ...
    const flashloanFee = (amount * 0.1 * FLASHLOAN_FEE_BPS) / 10000;
