...
// ⚠️ You are using a public RPC (triton.one). For production, use a private RPC like Helius or QuickNode.
...
bot.on('text', async ctx => {
  ...
  let msg = await ctx.reply('🔍 Searching… (0/3)');

  try {
    await ctx.editMessageText('🔍 Searching… (1/3) Getting token decimals');
    const dec = await getDec(mint);
    if (!dec) return ctx.editMessageText('❌ Token not found or no decimals');

    await ctx.editMessageText('🔍 Searching… (2/3) Quoting USDC → Token');
    const buyTokenQ = await jupQuote('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v', mint, Math.floor(SIZE_USD * 1e6));
    if (!buyTokenQ) return ctx.editMessageText('❌ No route: USDC → Token');

    const tokenOut = Number(buyTokenQ.data[0].outAmount) / (10 ** dec);

    await ctx.editMessageText('🔍 Searching… (3/3) Quoting Token → USDC');
    const sellQ = await jupQuote(mint, 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v', Math.floor(tokenOut * 10 ** dec));
    if (!sellQ) return ctx.editMessageText('❌ No route: Token → USDC');

    const routes = await build(mint, SIZE_USD);
    if (!routes.length || routes[0].profit <= 0) {
      return ctx.editMessageText(`📉 No profit after fees (${routes[0]?.profit?.toFixed(4) || 0} USDC)`);
    }
    ...
    logger.error(e);
    await ctx.editMessageText('❌ Search failed – check server logs');
  }
});
...
bot.action(/exec:(.+)/, async ctx => {
  ...
  await ctx.answerCbQuery(); // Acknowledge immediately
