...
# Map token addresses to CoinGecko IDs (subset for validation)
COINGECKO_IDS = {
...
class ArbDetector:
    ...
    def load_tokens(self):
        """Initialize with an empty token list; populated via refresh_tokens"""
        self.tokens = {
            ...
            'MOODENG': {'address': '8sUHD6b9kU7K67264m86o2Tog2xMPf3o3nR5Hwwa8rKn'},
            'WEN': {'address': '3mint6Q7xTusfK2K6mrXhHmt2aT6Nekn7W91A8sK3x'},
            'BOME': {'address': '9n4nbM75f5Ui33ZbPYXn59EwSgE8CGsHtAeTH5YFeJ9E'},
            'SLERF': {'address': '7BgBvyjrZX1YKz4ohE0mjb2AjFxFG1Dm1o9XHu7JYRPg'},
            'MANEKI': {'address': '3XJ3hW6F1YQGsWQQS3Yx9AuSbhgCRYgqYBF2dNAuJ4xy'},
            'MOG': {'address': 'Mog8U4pDxc58uX1MmxHgH3N4t41pwvXxt2Tq2pC4T6y'},
            'GME': {'address': '8wXtPeU6557ETKp3m4WcoQh5K8q7qA8PK6Kn4ggL2VU2'}
        }
    ...
    async def refresh_tokens(self, session, limit=50):
        """Fetch tokens from Solana Token List"""
        try:
            async with session.get(SOLANA_TOKEN_LIST_URL, timeout=20) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    new_tokens = {}
                    for token in data.get('tokens', [])[:limit]:
                        if token.get('chainId') == 101 and 'symbol' in token and 'address' in token:
                            # Filter for meme coins or tokens with CoinGecko IDs
                            if 'meme' in token.get('tags', []) or token['address'] in COINGECKO_IDS:
                                symbol = token['symbol']
                                new_tokens[symbol] = {'address': token['address']}
                    if new_tokens:
                        self.tokens = new_tokens
                        logger.info(f"Refreshed {len(new_tokens)} tokens from Solana Token List")
                    else:
                        logger.warning("No valid tokens from Solana Token List")
                else:
                    logger.error(f"Solana Token List fetch failed: {resp.status}")
        except Exception as e:
            logger.error(f"Solana Token List error: {type(e).__name__}: {e}")
...
async def refresh_tokens_endpoint():
    try:
        ...
        return jsonify({'status': 'success', 'message': 'Tokens refreshed from Solana Token List'})
    except Exception as e:
...
async def run_bot():
    detector = ArbDetector()
    async with aiohttp.ClientSession() as session:
        await detector.refresh_tokens(session)  # Initial token fetch
    while True:
        try:
            await detector.scan()
        except Exception as e:
