...
async function jupQuote(inputMint, outputMint, amountLamports, retries = 3) {
  const url = `https://quote-api.jup.ag/v6/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amountLamports}&slippageBps=${SLIPPAGE_BPS}&onlyDirectRoutes=false`;
...
// --------------  BUILD ROUTES  --------------
async function buildBestRoutes(tokenMint, amountSol = 0.1) {
  const decimals = await getTokenDecimals(tokenMint);
  if (!decimals) return [];

  const lamports = Math.floor(amountSol * 10 ** decimals);          // token lamports
  const solLamports = Math.floor(amountSol * 1e9);                  // 0.1 SOL
...
// --------------  EXECUTE  --------------
async function executeFlashloanTrade(tokenMint, buyDex, sellDex, amountSol = 0.1) {
  try {
    const decimals = await getTokenDecimals(tokenMint);
